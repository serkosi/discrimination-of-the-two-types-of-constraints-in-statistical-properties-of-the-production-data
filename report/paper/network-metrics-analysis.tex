...
Table~\ref{Tab: z-score_range_def} summarises possible network structures, including our assumptions for the respective z-score intervals under the effect of null model choice.
\begin{table}[ht!]
	\centering
	...
	\begin{tabular}{|c|c|c|}
		...
		$z\leq-2$ & \multirow{3}{*}[-1.25em]{\makecell{degrees sequence\\is conserved\\(NM-d)}} & \makecell{nonrandom,\\complicated}\\ \cline{1-1} \cline{3-3} 
		$-2<z<2$ &  & \makecell{random,\\not modular} \\ \cline{1-1} \cline{3-3} 
		$z\geq2$ &  & \makecell{nonrandom,\\modular} \\ \hline
		$z\leq-2$ & \multirow{3}{*}[-0.75em]{\makecell{degrees sequence\\and modules\\ are conserved\\(NM-m)}} & \makecell{complicated,\\ distorted/skewed-modular} \\ \cline{1-1} \cline{3-3} 
		$-2<z<2$ &  & \makecell{not complicated\\structure}\\ \cline{1-1} \cline{3-3} 
		$z\geq2$ &  & \makecell{complicated,\\hierarchical organization} \\ \hline
	\end{tabular}
